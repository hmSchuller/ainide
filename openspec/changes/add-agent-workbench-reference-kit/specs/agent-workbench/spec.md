...
The system SHALL provide an Agents mode alongside the existing Edit and Review modes. Agents mode SHALL present a dedicated workbench for agent sessions rather than relying on the resizable terminal footer, and switching modes SHALL NOT terminate live agent sessions.

#### Scenario: User opens the Agents mode

- **WHEN** the user selects Agents from the primary mode controls
- **THEN** the workbench shows the active project's agent session navigator and terminal workspace

#### Scenario: User leaves Agents mode

- **WHEN** the user switches from Agents to Edit or Review
...
The system SHALL keep shell, Lazygit, and custom terminal sessions accessible from the Agents workbench as secondary tools without presenting them as agent sessions.

#### Scenario: User opens a shell while supervising agents

- **WHEN** the user requests a shell or other non-agent terminal from Agents mode
- **THEN** the tool session can be selected and used without removing or replacing any agent session

### Requirement: Agents are scoped to the active project

The Agents workbench SHALL display only the agent and tool sessions belonging to the active project. Sessions belonging to hidden open projects SHALL continue running, and switching back to that project SHALL show its existing sessions rather than creating duplicates.
...
- **THEN** the Agents workbench lists those existing sessions with their existing identities and output
