import { copyReference, copyReferenceKit, serializeReferenceKit } from "../references";
...
export function ReferenceDock() {
  ...
  const pasteKit = async () => {
    ...
    if (!target) { setNotice("No live agent is available for handoff; use Copy kit instead", "error"); return; }
    try {
  ...
  return <aside className="reference-dock" aria-label="Reference kit">
    <header><div><span className="eyebrow">REFERENCE KIT</span><strong>{kit.length} {kit.length === 1 ? "item" : "items"}</strong></div>{kit.length > 0 && <button onClick={clearReferences}>Clear</button>}</header>
    {kit.length === 0 ? <p className="reference-empty">Select code or use a file action to collect context here.</p> : <div className="reference-items">{kit.map((item) => <div className="reference-item" key={item.id}><div className="reference-item-copy"><strong>{item.path}</strong><span>{item.wholeFile ? "whole file" : `lines ${item.startLine}-${item.endLine}`}</span><small>{item.content.length.toLocaleString()} chars</small></div><div className="reference-item-actions"><button onClick={() => void copyItem(item.id)} title="Copy this reference">Copy</button><button onClick={() => removeReference(item.id)} title="Remove reference">×</button></div></div>)}</div>}
    <div className="reference-controls"><label htmlFor="dock-agent-target">Target agent</label><select id="dock-agent-target" value={target?.id ?? ""} onChange={(event) => setTarget(event.target.value || undefined)}><option value="">No live agent</option>{liveAgents.map((agent) => <option value={agent.id} key={agent.id}>{agent.title}</option>)}</select><div className="reference-buttons"><button onClick={() => void copyKit()} disabled={!kit.length}>Copy kit</button><button className="primary-button" onClick={() => void pasteKit()} disabled={!kit.length}>Paste reference kit</button></div></div>
  </aside>;
}
